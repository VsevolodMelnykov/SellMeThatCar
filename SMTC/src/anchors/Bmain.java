...
public class Bmain extends Main{
	Auction a;
	Accepted aa;
	Confirmed aaa;
	int bid = 0;
    ...
    void ab(ActionEvent event) throws FileNotFoundException, ClassNotFoundException, IOException {
    	if (act.isDisabled()) {
    		...
    		if (a != null) {
        		...
        		id.setText(a.getId());
            	if (a.bid.size() != 0) {
            		bidd.setText(Integer.toString(a.bid.get(bid)));
            		biddd.setText(a.bidder.get(bid).getId());
            	}
            	else {
            		bidd.setText("Bid");
    ...
    void ca(ActionEvent event) throws IOException {
    	...
    	stage.close();
    	java.net.URL url = new File("C:\\Users\\Ctdf6\\eclipse-workspace\\SMTC\\src\\anchors\\bcreateauction.fxml").toURI().toURL();
